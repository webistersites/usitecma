(function($) {
	$(document).ready(function() {
		...
		function legal_add_file(event, selector) {
			...
			frame.on( 'select', function() {
				...
				selector.find('.remove-image, .remove-file').on('click', function() {
					legal_remove_file( $(this).parents('.section') );
				});
			});
		...
		function legal_remove_file(selector) {
			...
			selector.find('.upload-button').live('click', function() {
				legal_add_file(event, $(this).parents('.section'));
			});
		}
